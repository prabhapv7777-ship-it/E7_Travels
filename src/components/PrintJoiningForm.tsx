...
export default function PrintJoiningForm({ enquiry, onClose }: PrintJoiningFormProps) {
  ...
  return createPortal(
    <div className="print-modal-root fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-xs flex flex-col items-center justify-start overflow-y-auto p-4 sm:p-6 md:p-10 print:p-0 print:bg-white print:static print:overflow-visible">
      ...
      {printError && (
        <div className="w-full max-w-[850px] bg-rose-500 text-white p-3.5 text-xs font-extrabold rounded-lg mb-2 flex items-center justify-between border-2 border-rose-600 print:hidden shadow-md">
          <div className="flex items-center gap-2 text-left">
      ...
      <div className="w-full max-w-[850px] bg-slate-800 text-white rounded-t-xl p-4 flex flex-wrap gap-4 items-center justify-between shadow-lg print:hidden">
      ...
      <div className="w-full max-w-[850px] bg-slate-100 flex flex-col md:flex-row shadow-2xl overflow-visible print:shadow-none print:bg-white print:block">
        ...
        {/* Printable Paper A4 Layout Sheet (Right side on screen, full printable sheet on print) */}
        <div 
          ref={printAreaRef}
          className="print-sheet flex-1 bg-white p-6 sm:p-10 font-sans text-slate-900 border-none relative print:p-0 print:m-0 print:w-full print:shadow-none"
        ...
        >
          ...
          <style dangerouslySetInnerHTML={{ __html: `
            ...
            @media print {
              ...
              /* Position print sheet perfectly in top-left with minimal margins to fit exactly one page */
              @page {
                size: A4 portrait;
                margin: 0.5cm 0.8cm 0.5cm 0.8cm !important;
              }
              html, body {
                background: white !important;
              ...
              .print-container.space-y-5 {
                gap: 0.45rem !important;
              }
              .print-container.space-y-5 > :not([hidden]) ~ :not([hidden]) {
                margin-top: 0.45rem !important;
              }
              .print-container .space-y-2 > :not([hidden]) ~ :not([hidden]) {
                margin-top: 0.2rem !important;
              }
              .print-container .space-y-3 > :not([hidden]) ~ :not([hidden]) {
                margin-top: 0.2rem !important;
              }
              ...
              /* Header resizing */
              .print-container img, .print-container .h-14 {
          ...
          <div className="print-container space-y-5 text-left text-xs leading-relaxed">
            
            {/* Header branding */}
            <div className="flex flex-col items-center border-b-2 border-slate-200 pb-3">
              <div className="flex flex-col items-center text-center">
                {customLogo ? (
                  ...
                  <div className="h-16 w-16 rounded-full bg-amber-500 flex items-center justify-center text-slate-950 font-black text-2xl border-2 border-slate-950 shadow-sm shrink-0 mb-2">
                    E7
                  </div>
                )}
                <h1 className="font-serif font-black tracking-widest text-amber-600 uppercase leading-none" style={{ fontSize: '22px' }}>
                  E7 TRAVELS
                </h1>
              </div>

              <div className="w-full flex justify-between items-center text-xs mt-3 pt-1 border-t border-slate-100">
                <div className="flex items-center gap-1.5">
                  <span className="font-bold text-slate-500 uppercase">DATE:</span>
                  <span className="font-bold px-2 py-0.5 bg-slate-100 border border-slate-300 rounded text-center min-w-[100px] inline-block font-mono">
                    {formatDateToDDMMYYYY(formData.enquiryDate) || '\u00A0'}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="font-bold text-slate-500 uppercase">APPLICATION NO:</span>
                  <span className="font-black px-2 py-0.5 bg-slate-100 border border-slate-300 rounded font-mono text-sm min-w-[100px] text-center inline-block">
                    {formData.id || '\u00A0'}
                  </span>
                </div>
              </div>
            ...
            <div className="text-center py-1">
              <h2 className="text-xl font-extrabold text-[rgb(0,200,83)] tracking-wider uppercase">VEHICLE JOINING APPLICATION FORM</h2>
            </div>

            {/* SECTION 1: INDUCTION CATEGORY & DETAILS */}
            <div className="space-y-1.5">
              <div className="bg-slate-100 border-l-[5px] border-red-600 px-3 py-1 flex justify-between items-center">
                <h3 className="font-black text-[rgb(0,200,83)] text-xs uppercase tracking-wider">
                  1. INDUCTION CATEGORY & DETAILS
                </h3>
              </div>
              <div className="border border-slate-300 rounded-sm overflow-hidden divide-y divide-slate-300 bg-white text-xs">
                {/* Row 1 */}
                <div className="grid grid-cols-2 divide-x divide-slate-300">
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    <div className="w-36 bg-slate-50 px-2 py-1 flex items-center font-bold text-slate-600 text-[10px] shrink-0">
                      Induction Type:
                    </div>
                    <div className="flex-1 bg-white px-2 py-1 flex items-center gap-3">
                      <label className="inline-flex items-center gap-1 cursor-pointer">
                        ...
                        />
                        <span className="text-[10px] font-bold text-slate-800">OwnerAttach</span>
                      </label>
                      <label className="inline-flex items-center gap-1 cursor-pointer">
                        <input 
                           type="checkbox" 
                           ...
                           className="h-3 w-3 rounded border-slate-300 text-amber-500 focus:ring-0" 
                        />
                        <span className="text-[10px] font-bold text-slate-800">DriverAttach</span>
                      </label>
                    </div>
                  </div>
                  <div className="flex bg-white min-h-[30px]"></div>
                </div>
                ...
                <div className="grid grid-cols-2 divide-x divide-slate-300">
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    ...
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-mono font-bold border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900">
                        {formData.ownerId || '\u00A0'}
                      </span>
                    </div>
                  </div>
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    ...
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-bold border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900 truncate">
                        {formData.ownerName || '\u00A0'}
                      </span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 divide-x divide-slate-300">
                  ...
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    ...
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-bold border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900 italic">
                        {formData.reference || '\u00A0'}
                      </span>
                    </div>
                  </div>
              ...
              <div className="border border-slate-300 rounded-sm overflow-hidden divide-y divide-slate-300 bg-white text-xs">
                ...
                <div className="grid grid-cols-2 divide-x divide-slate-300">
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    ...
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-mono font-black border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900">
                  ...
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    ...
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-bold border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900">
              ...
              <div className="border border-slate-300 rounded-sm overflow-hidden divide-y divide-slate-300 bg-white text-xs">
                ...
                <div className="grid grid-cols-2 divide-x divide-slate-300">
                  ...
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    ...
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-mono font-bold border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900 uppercase">
                ...
                <div className="grid grid-cols-2 divide-x divide-slate-300">
                  ...
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    <div className="w-36 bg-slate-50 px-2 py-1 flex items-start font-bold text-slate-600 text-[10px] shrink-0 mt-0.5">
                      Permanent Address:
                    </div>
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-bold border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900 leading-tight">
            ...
            <div className="space-y-1.5">
              ...
              <div className="border border-slate-300 rounded-sm overflow-hidden divide-y divide-slate-300 bg-white text-xs">
                ...
                <div className="grid grid-cols-2 divide-x divide-slate-300">
                  <div className="flex divide-x divide-slate-300 min-h-[30px]">
                    <div className="w-24 bg-slate-50 px-2 py-1 flex items-center font-bold text-slate-600 text-[10px] shrink-0">
                      Site 1:
                    </div>
                    <div className="flex-1 bg-white px-2.5 py-1 flex items-center">
                      <span className="font-bold border-b border-slate-300 flex-1 px-1 min-h-[16px] text-left text-[11px] text-slate-900">
            ...
            <div className="space-y-2 pt-1">
              ...
              <p className="text-[10.5px] leading-relaxed text-slate-700 text-justify font-medium">
              ...
              {/* Signatures */}
              <div className="flex justify-between items-end pt-12 pb-2">
                <div className="text-center w-[280px]">
                  <div className="border-t border-dotted border-slate-400 pt-1.5 text-[10px] font-bold text-slate-600 uppercase">
            ...
            <div className="bg-amber-50/20 border border-amber-300/60 p-3 rounded-lg text-[9.5px] leading-relaxed text-amber-900/90 font-medium">
