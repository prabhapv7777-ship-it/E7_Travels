...
export default function MasterViews({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      {activeSubView === 'Vehicle Master' && !isAdding && !editingId && (
      ...
      {(isAdding || editingId) && (
        <div className="p-6 border-b border-slate-200 bg-blue-50/10">
          <h3 className="text-sm font-semibold text-slate-700 uppercase tracking-wider mb-4">
            {editingId ? 'Edit Register Details' : 'Add New Entry'}
          </h3>
              ...
              {filteredVehicles.map((v) => {
                ...
                return (
                  <tr key={v.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="py-3 px-4 text-xs text-slate-600 truncate max-w-[120px]">{v.company}</td>
